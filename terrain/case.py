...
class Case():
    ...
    def update(self):
        if self.content =="UNIT":
            shape =  shapes.BorderedRectangle(self.get_x(update=True), self.get_y(update=True), BLOCKSIZE, BLOCKSIZE, border=3, color=(0, 50, 0),
                                     border_color=(255, 0, 0), batch=self.batch, group=self.group)
            shape.opacity = 100
            self.shape = shape

        if self.content =="NONE":
            shape =  shapes.BorderedRectangle(self.get_x(update=True), self.get_y(update=True), BLOCKSIZE, BLOCKSIZE, border=3, color=(0, 0, 0),
                                     border_color=(255, 0, 0), batch=self.batch, group=self.group)
            shape.opacity = 100
            self.shape = shape
